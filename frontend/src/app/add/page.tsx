  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="glass rounded-2xl p-6 sm:p-8">
        ...
        {result && (
          <div
            className={`mt-5 flex items-start gap-3 p-4 rounded-xl ${result.success
          ...
          >
            ...
            <p
              className={`text-sm ${result.success ? "text-success" : "text-red-400"
                }`}
            >
              {result.message}
            </p>
      ...
      <div className="mt-8 glass rounded-2xl p-6">
        <div className="flex items-start gap-4 mb-5">
          ...
          <div>
            <h3 className="text-sm font-semibold text-foreground mb-1">
              Save via WhatsApp Bot
            </h3>
            <p className="text-xs text-muted leading-relaxed">
              Forward any Instagram, Twitter, or article link to the bot — it
              auto-analyzes and saves it to your dashboard within seconds.
            </p>
          </div>
        </div>
        ...
        <div className="bg-green-500/8 border border-green-500/20 rounded-xl p-4 mb-4">
          <p className="text-xs text-muted uppercase tracking-wide mb-1 font-medium">Bot Number</p>
          <a
          ...
          >
            <p className="text-lg font-bold text-green-400 tracking-wide group-hover:text-green-300 transition-colors underline underline-offset-4 decoration-green-500/40">
              +1 (415) 523-8886
            </p>
            <ExternalLink className="w-4 h-4 text-green-500/60 group-hover:text-green-400 transition-colors" />
          </a>
          <p className="text-xs text-muted mt-1">Tap to open in WhatsApp · Twilio Sandbox</p>
        </div>

        <div className="space-y-2">
          {[
            { step: "1", text: 'Open WhatsApp and message the number above' },
            { step: "2", text: 'Send: join development-exclaimed' },
            { step: "3", text: 'Now just forward any link — the bot saves it instantly!' },
          ].map((s) => (
            <div key={s.step} className="flex items-start gap-3">
              <span className="w-5 h-5 rounded-full bg-green-500/20 text-green-400 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                {s.step}
              </span>
